defmodule Skn.DB.Bot do
    ...
    def gen_android_id(device) do
        ...
        id = case device do
        :ioscn ->
            UUID.uuid4() |> String.upcase()
        :ios ->
            UUID.uuid4() |> String.upcase()
        _ ->
            a = :crypto.strong_rand_bytes(8)
            a |> Base.encode16(case: :lower)
        end
    ...
    def gen_player_name() do
        ...
        if is_gen_name == true do
            l = Enum.random([5, 6, 7, 8, 9])
            (Enum.map 0..l, fn(_)-> :rand.uniform(24) + 96 end) |> :binary.list_to_bin |> String.capitalize
        else
    ...
    def gen_league_name() do
        ...
        l2 = Enum.random([1, 2])
        a1 = (Enum.map 0..l1, fn(_)-> Enum.random(97..122) end) |> :binary.list_to_bin
        a2 = (Enum.map 0..l2, fn(_)-> Enum.random(48..57) end) |> :binary.list_to_bin
        (a1 <> a2) |> String.capitalize
    end
    ...
    def gen_mac_address2() do
        ...
        mac = (mac |> :erlang.binary_to_list |> Enum.map(&(Base.encode16(<<&1>>, case: :lower)))  |> Enum.join(":"))
    ...
    def get(botid) do
        case :mnesia.dirty_read(:bot_record, botid) do
            [r|_] ->
                %{id: Skn.Bot.Repo.bot_record(r, :id), config: Skn.Bot.Repo.bot_record(r, :config),
    ...
    def bot_update_conf(botid, data) do
        ...
        data = Map.drop(data, [:proxy2, :proxy_auth2, :proxy, :proxy_auth, :proxy_auth_fun, :proxy_keep_alive, :eam_session, :code, :game_code, :eformula,
        :boot_time, :ah_time])
        case :mnesia.dirty_read(:bot_record, botid) do
            [r|_] ->
    ...
    def bot_update_conf2(botid, data) do
        # remove temporary key
        data = Map.drop(data, [:proxy2, :proxy_auth2, :proxy, :proxy_auth, :proxy_auth_fun, :eam_session, :code, :game_code, :eformula,
        :boot_time, :ah_time])
